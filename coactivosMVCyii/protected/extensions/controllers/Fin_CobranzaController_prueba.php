...
class Fin_CobranzaController extends Controller
{
	...
	{
		$model = new CActiveDataProvider('fin_Cobranza',array());
		...
		if(isset($_POST['uprc_buscarrdeudas']))
		{
			$dataProvider = new CActiveDataProvider('ven_Deuda',array(
				'criteria'=>array(
					'condition'=>'saldo > 0 and cliente_id='.$_POST['cliente_id'],
				),
			));
		}
	...
	{		
		...
		{	
			...
			$cobranza->importe = 0;
			$cobranza->complemento_idMoneda = 8;
			$cobranza->estado_idDeuda = 1;
			$cobranza->fecha = new CDbExpression('NOW()');
			...
			if(!$cobranza->validate())
			{
				...
				foreach($items as $item)
				{
					...
					$idcliente=$_POST['cliente_id'];
						...
						$deuda->saldo = $deuda->saldo - $deuda->importe;
	...
	public function actionListarClientes($term)
	{
        ...
        foreach ($data as $item) {
			...
			//'value' => $item->descripcion1.' '.$item->descripcion2,
